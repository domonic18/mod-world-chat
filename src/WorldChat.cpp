    ...
    for (SessionMap::iterator itr = sessions.begin(); itr != sessions.end(); ++itr)
    {
        ...
        if (WorldChat[guid2].chat == 1 && (team == -1 || target->GetTeamId() == team))
        {
            if (WC_Config.CrossFaction || (sender->GetTeamId() == target->GetTeamId()) || target->GetSession()->GetSecurity())
            {
                ...
                {
                    std::string teamname = "";
                    if (sender->GetTeamId() == TEAM_HORDE)
                    {
                        teamname = "部落";
                    }
                    else
                    ...
                    snprintf(message, 1024, "[世界][%s][%s|Hplayer:%s|h%s|h|r]: %s%s|r", teamname.c_str(), world_chat_ClassColor[sender->getClass() - 1].c_str(), sender->GetName().c_str(), sender->GetName().c_str(), WORLD_CHAT_WHITE.c_str(), msg);
                }
